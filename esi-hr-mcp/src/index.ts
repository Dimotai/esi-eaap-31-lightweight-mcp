...
const defaultScoreThreshold = Number(
  process.env.HR_KB_DEFAULT_SCORE_THRESHOLD || "0.0"
);
...
// Register the retrieve_hr_policy tool

type RetrieveHrPolicyInput = {
  query: string;
  topK?: number;
...
server.registerTool(
  ...
  {
    description:
      "Searches the ESI HR knowledge base (handbook, PTO, benefits, etc.) and returns ranked chunks + scores.",
    inputSchema: {
      query: z
        .string()
  ...
  async (input: RetrieveHrPolicyInput) => {
    const { query, topK, scoreThreshold } = input;

    const effectiveTopK = topK ?? defaultTopK;
    const effectiveThreshold =
      scoreThreshold ?? (!Number.isNaN(defaultScoreThreshold) ? defaultScoreThreshold : undefined);
